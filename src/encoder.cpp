...
Int BonkEnc::BonkEnc::Encoder(Thread *thread)
{
	...
	for (Int i = 0; i < num; (step == 1) ? i++ : i)
	{
		...
		if (trackInfo->outfile == NIL)
		{
			...
			else if (currentConfig->encoder == ENCODER_FLAC)	out_filename.Append(".flac");
		...
		int		 lastpercent	= 100;
		Driver		*driver_out	= new DriverZero();
		...
		if (encoder == ENCODER_FLAC)		filter_out = new FilterOutFLAC(currentConfig, trackInfo);
		...
		if (f_out->AddFilter(filter_out) == true)
		{
			startticks = clock();
