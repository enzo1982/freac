...
const int sample_shift   = 4;
const int lattice_factor = 1 << lattice_shift;
const int sample_factor  = 1 << sample_shift;

//Maximum allowable taps
const int max_tap        = 2048;

//Default quantization level
const double base_quant = 0.6;

//Amount of bit rate variation
const double rate_variation = 3.0;

const int tap_quant[max_tap] = {
...
struct lattice
{
	...
	void dequantize()
	{ 
		for (int i = 0; i < order; i++) k[i] *= tap_quant[i];
	}
